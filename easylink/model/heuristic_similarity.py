...
if __name__ == '__main__':
    # toy data
    num_nodes = 6
    edge_index_raw = np.array(
        [[0, 1], [1, 2], [1, 5], [2, 3], [2, 4], [3, 4], [4, 5]])
    edge_index_reverse = np.array(
        [edge_index_raw[:, 1], edge_index_raw[:, 0]]).transpose()
    edge_index = np.concatenate([edge_index_raw, edge_index_reverse], axis=0)
    edge_weight = torch.ones(edge_index.shape[0], dtype=int)
    A = ssp.csr_matrix((edge_weight, (edge_index[:, 0], edge_index[:, 1])), shape=(
        num_nodes, num_nodes))
